...
public class RmdCommandTests
{
    ...
    public RmdCommandTests()
    {
        _context = new FtpCommandContext(_session, "RMD", "olddir", _fileSystem, _authenticator, new(), NullLogger.Instance);
    ...
    {
        var sut = new RmdCommand();
        Assert.Equal(["RMD"], sut.Verbs);
    ...
    [Fact]
    public async Task ExecuteAsync_WhenDirectoryExists_ShouldRemoveAndReturn250()
    {
        var sut = new RmdCommand();
        _fileSystem.DirectoryExistsAsync(Arg.Any<FtpAuthenticationContext>(), "/olddir").Returns(true);
        await sut.ExecuteAsync(_context);
        await _fileSystem.Received().DeleteDirectoryAsync(Arg.Any<FtpAuthenticationContext>(), "/olddir");
    ...
    {
        var sut = new RmdCommand();
        _fileSystem.DirectoryExistsAsync(Arg.Any<FtpAuthenticationContext>(), "/olddir").Returns(false);
        await sut.ExecuteAsync(_context);
    ...
    {
        var sut = new RmdCommand();
        _fileSystem.DirectoryExistsAsync(Arg.Any<FtpAuthenticationContext>(), "/olddir").Throws(new InvalidOperationException());
        await sut.ExecuteAsync(_context);
        await _session.Received().SendResponseAsync(550, Arg.Any<string>());
    }
}
